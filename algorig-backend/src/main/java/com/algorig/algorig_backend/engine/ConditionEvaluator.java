...
public class ConditionEvaluator {
    ...
    public boolean evaluate(String condition, BattleContext context, RobotMemory memory) {
        ...
        if (trimmed.contains(" OR ")) {
            for (String part : trimmed.split(" OR ")) {
                if (evaluate(part.trim(), context, memory)) return true;
            }
            return false;
        }
        ...
        if (trimmed.contains(" AND ")) {
            for (String part : trimmed.split(" AND ")) {
                if (!evaluate(part.trim(), context, memory)) return false;
            }
            return true;
        }
        ...
        if (trimmed.startsWith("NOT ")) {
            return !evaluate(trimmed.substring(4).trim(), context, memory);
        }

        return evaluateSimple(trimmed, context, memory);
    }

    private boolean evaluateSimple(String condition, BattleContext context, RobotMemory memory) {
        String[] operators = {"<=", ">=", "==", "!=", "<", ">"};
        for (String op : operators) {
            int idx = condition.indexOf(op);
            if (idx != -1) {
                int left  = resolveValue(condition.substring(0, idx).trim(), context, memory);
                int right = resolveValue(condition.substring(idx + op.length()).trim(), context, memory);
                return switch (op) {
                    case "<"  -> left < right;
                    case ">"  -> left > right;
                    case "<=" -> left <= right;
    ...
    private int resolveValue(String token, BattleContext context, RobotMemory memory) {
        return switch (token) {
            ...
            default -> {
                // Check user memory variables first
